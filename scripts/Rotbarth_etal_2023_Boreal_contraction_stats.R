#############################Introduction######################################
# R script containing data preparation, exploration, visualisation and analysis for the paper titled 'Tree cover changes reveal contraction of North American boreal forests' published in Nature Communications 2022
# 
...
# 
# R script version dated 22 November 2022
# 
# All analysis were performed in RStudio Version 1.1.463
# 
# R version: 4.2.0
# 
# Abstract: Climate change is expected to shift the boreal biome northward through expansion at the northern and contraction at the southern boundary respectively. However, biome-scale evidence of such a shift is rare. We used remotely-sensed tree cover data to quantify temporal changes across the North American boreal biome over the past two decades (2000-2019). We found a strong north-south asymmetry in tree cover change, consisting of a mean zero change near the northern and losses of 0.13% per year at the southern boundary. Tree cover losses due to wildfires and timber logging substantially reduced tree cover by up to 0.3% per year in southern regions, where recovery seemed to be reduced under the combined effects of climate warming and disturbances. The observed asymmetry in tree cover change indicates a possible transient contraction of the boreal biome, which could lead to a net release of carbon on a biome scale and accelerate warming trends in high latitudes. Continuous monitoring is required to assess climate change related transient biome dynamics that are unexpected from long-term model predictions. 
# 
# 



#### 1. Read libraries and working directory####
library(tidyverse)
library(lattice)
library(ggplot2)
library(mgcv)
...
#### 3. Read in dataset####
boreal = read.csv("NA_boreal_raw_data.csv", header = T)

str(boreal)

boreal$GLC = as.factor(boreal$GLC) #land cover type as factor
oldglc.levels = levels(boreal$GLC)

boreal$GLC = factor(boreal$GLC, levels = c("Non-woody", "Shrubs", "Needleleaf forest", "Mixed forest", "Broadleaf forest", "Unknown forest"))

boreal$boundary = as.factor(boreal$boundary) 
boreal$ecozone = as.factor(boreal$ecozone) 

boreal$dist_type3 = as.factor(boreal$dist_type3)
boreal$dist_type3 = factor(boreal$dist_type3, levels = c("Disturbed < 1985/Undisturbed", "Fire 1985-1999", "Harvest 1985-1999", "Fire 2000-2019", "Harvest 2000-2019"))

#mean tree cover change
boreal$meantc = boreal %>% 
  select(c(mean2000:mean2019)) %>%
  rowMeans()

#relative tree cover change as a function of absolute tree cover change and mean tree cover
boreal$reltrend = boreal$trend.yp*100/boreal$meantc

#Boreal distribution boundaries as factors
boreal$flon = as.factor(boreal$lon) #transect longitudes as factor

#order boundary categories from north to south
boreal$boundary = as.factor(boreal$boundary) 
boreal$boundary = factor(boreal$boundary, levels = c("N-North", "S-North", "N-Interior", "S-Interior", "N-South", "S-South"))




#### 4. Data exploration: Correlation between predictors ####
variables = select(boreal, c(distance,elev,meantc,temp,meanpre,tchange8019,tschange8019,pchange8019,tchange0019,tschange0019,pchange0019))
...
env_names = c("SBD", "Elevation", "Mean TC","MAT", "MAP", "trend MAT80-19", "trend MST80-19", "trend MAP80-19", "trend MAT00-19", "trend MST00-19",  "trend MAP00-19")

variables$trend.class = "1"
variables$trend.class[boreal$trend.yp < 0 & boreal$sig.yp <= 0.05] = "3"
variables$trend.class[boreal$trend.yp > 0 & boreal$sig.yp <= 0.05] = "2"

#variables$trend.yp = variables$trend.yp*100/boreal$meantc

pca = prcomp(select(variables, -trend.class), scale. = T)
...
# Both the correlation plot and the PCA plot show strong correlations between some predictor variables. We will therefore perform models for each predictor separately. 


#### 5. Statistical modeling of tree cover change ####
#### 5.1 Generalised additive mixed-effects models####
...
canada = subset(boreal, boreal$ecozone != "Alaska")

ggplot(data = canada, aes(x = flon,
                          y = trend.yp)) +
...
ICC <- 0.34872084 ^2 / (0.34872084 ^2 + 0.09411899 ^2)
ICC
...
canada$E1  <- resid(gamm.distance)
canada$mu1 <- fitted(gamm.distance)



#' Pearson residuals versus fitted values.
p1 <- ggplot(canada, aes(x = mu1, y = E1))+
  geom_point(size = 1)+
  geom_hline(yintercept = 0, 
             linetype="dashed",
             color = "red")+
...
p2 <- ggplot(canada, aes(x = dist_type3, y = E1))+
  geom_boxplot()+
...
p3 <- ggplot(canada, aes(x = mu1, y = trend.yp))+
...
p4 <- ggplot(canada, aes(x = distance, y = E1))+
  geom_point()+
  geom_smooth()+
  geom_hline(yintercept = 0, color = "red")+
...
p5 <- ggplot(canada, aes(x = flon, y = E1))+
  geom_boxplot()+
  geom_hline(yintercept = 0, color = "red")+
...
#### 5.1.2 Check for spatial correlation of model residuals####
...
#### 5.2 GAMM with spatial correlation structure: model fitting of relative and absolute tree cover trends and environmental variables #### 
#### 5.2.1 Model RELATIVE TREE COVER and.... ####
#...distance by disturbance and vegetation type
canada = subset(boreal, boreal$ecozone != "Alaska")

canada$dist_GLC = interaction(canada$dist_type3, canada$GLC)
...
#' Get residuals and fitted values:
canada$E01  <- resid(M01$gam)
canada$mu01 <- fitted(M01$gam)
canada$E1  <- resid(M1$gam)
canada$mu1 <- fitted(M1$gam)
canada$E2  <- resid(M2$gam)
canada$mu2 <- fitted(M2$gam)
canada$E3  <- resid(M3$gam)
canada$mu3 <- fitted(M3$gam)
canada$E4  <- resid(M4$gam)
canada$mu4 <- fitted(M4$gam)
canada$E5  <- resid(M5$gam)
canada$mu5 <- fitted(M5$gam)
canada$E6  <- resid(M6$gam)
canada$mu6 <- fitted(M6$gam)
canada$E7  <- resid(M7$gam)
canada$mu7 <- fitted(M7$gam)
canada$E8  <- resid(M8$gam)
canada$mu8 <- fitted(M8$gam)
canada$E9  <- resid(M9$gam)
canada$mu9 <- fitted(M9$gam)
...
#' Fitted values versus observed data.
p3 <- ggplot(canada, aes(x = mu1, y = reltrend))+
  geom_point(size = 0.5, col= grey(0.5))+
  geom_abline(intercept = 0, slope = 1, color = "red")+
  labs(x = "Fitted values", y = "Observed Tree cover change")+
  theme(text = element_text(size = 10))
...
#### 5.2.1.2 Model prediction and visualisation####
#Create table with min and max values for each predictor variables in the models and for each disturbance type
glcminmax = ddply(canada, c("GLC", "dist_type3"), summarise,
                  mindist = min(distance),
                  maxdist = max(distance))


climminmax = ddply(canada, "dist_type3", summarise,
                   mint = min(temp),
                   maxt = max(temp),
                   minp = min(meanpre),
                   maxp = max(meanpre),
...
for (i in 1:length(levels(canada$dist_type3))) {
  print(paste("Loop", i, "/", length(levels(canada$dist_type3))))
...
for (i in 1:length(levels(canada$dist_GLC))) {
...
dummy = data.frame(temp = unlist(tempdist),
                   ...
                   dist_type3 = as.factor(rep(c(levels(canada$dist_type3)), each = 100)))

dummy$dist_type3 = factor(dummy$dist_type3, levels = levels(canada$dist_type3))
...
dummyglc = data.frame(distance = unlist(distglc),
                      GLC = as.factor(rep(levels(canada$GLC), each = 500)),
                      dist_type3 = as.factor(rep(c(levels(canada$dist_type3)), each = 100)))

dummyglc$GLC = factor(dummyglc$GLC, levels = levels(canada$GLC))
dummyglc$dist_type3 = factor(dummyglc$dist_type3, levels = levels(canada$dist_type3))
dummyglc$dist_GLC = interaction(dummyglc$dist_type3, dummyglc$GLC)
dummyglc$dist_GLC = factor(dummyglc$dist_GLC, levels = levels(canada$dist_GLC))
...
glcrelplot = 
  ...
  geom_point(data = canada, aes(distance, reltrend, col = dist_type3), pch = 1, alpha = .4)+
...
distplot = 
  ...
  geom_point(data = canada, aes(distance, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(distance, fdist + sdist), lwd = .5)+
  geom_line(aes(distance, fdist - sdist), lwd = .5)+
...
matplot = 
  ...
  geom_point(data = canada, aes(temp, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
mapplot = 
  ...
  geom_point(data = canada, aes(meanpre, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
tcplot = 
  ...
  geom_point(data = canada, aes(meantc, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
elevplot.rel = 
  ...
  geom_point(data = canada, aes(elev, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
ttplot = 
  ...
  geom_point(data = canada, aes(tchange8019, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
tpplot = 
  ...
  geom_point(data = canada, aes(pchange8019, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
tt0019plot = 
  ...
  geom_point(data = canada, aes(tchange0019, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
tp0019plot = 
  ...
  geom_point(data = canada, aes(pchange0019, reltrend), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
...
finalplot = grid.arrange(distplot, tcplot, elevplot.rel, matplot, mapplot, ttplot, tpplot, tt0019plot, tp0019plot, ncol = 2,
...
glcrelplot = 
  ggplot(dummyglc, aes(distance, fdistrel))+
  geom_ribbon(data = dummyglc, aes(x = distance, ymin = fdistrel-sdistrel, ymax = fdistrel+sdistrel, fill = dist_type3), alpha = .4)+
  geom_line(aes(col = dist_type3), lwd = 1.3)+
  # geom_line(aes(distance, fdist + sdist), lwd = .5)+
  # geom_line(aes(distance, fdist - sdist), lwd = .5)+
  # 
...
distplot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = distance, ymin = fdist-sdist, ymax = fdist+sdist, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(distance, fdist, col = dist_type3), lwd = 1.3)+
  
  scale_color_manual(name = "",
                     values = distcol, 
                     labels = levels(dummy$dist_type3))+
  scale_fill_manual(name = "",
                    values = distcol, 
                    labels = levels(dummy$dist_type3))+
...
matplot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = temp, ymin = ftemp-stemp, ymax = ftemp+stemp, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(temp, ftemp, col = dist_type3), lwd = 1.3)+
...
mapplot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = meanpre, ymin = fpre-spre, ymax = fpre+spre, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(meanpre, fpre, col = dist_type3), lwd = 1.3)+
...
tcplot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = meantc, ymin = ftc-stc, ymax = ftc+stc, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(meantc, ftc, col = dist_type3), lwd = 1.3)+
...
elevplot.rel = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = elev, ymin = felev-selev, ymax = felev+selev, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(elev, felev, col = dist_type3), lwd = 1.3)+
...
ttplot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = tchange8019, ymin = ftt-stt, ymax = ftt+stt, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(tchange8019, ftt, col = dist_type3), lwd = 1.3)+
  ...
  labs(x = expression(paste('MAT change 1980-2019 (°C ' ~year^-1, ')')),
       y = expression(paste('TC change (% ' ~"%"^-1 ~year^-1, ')')),
       tag = "F")+
  theme_classic()+
  theme(legend.key.width = unit(2, units = "cm"),
...
tpplot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = pchange8019, ymin = ftp-stp, ymax = ftp+stp, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(pchange8019, ftp, col = dist_type3), lwd = 1.3)+
  ...
  labs(x = expression(paste('MAP change 1980-2019 (mm ' ~year^-1, ')')),
       y = expression(paste('TC change (% ' ~"%"^-1 ~year^-1, ')')),
       tag = "G")+
  theme_classic()+
  theme(legend.key.width = unit(2, units = "cm"),
...
tt0019plot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = tchange0019, ymin = ftt0019-stt0019, ymax = ftt0019+stt0019, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(tchange0019, ftt0019, col = dist_type3), lwd = 1.3)+
  ...
  labs(x = expression(paste('MAT change 2000-2019 (°C ' ~year^-1, ')')),
       y = expression(paste('TC change (% ' ~"%"^-1 ~year^-1, ')')),
       tag = "H")+
  theme_classic()+
  theme(legend.key.width = unit(2, units = "cm"),
...
tp0019plot = 
  ggplot()+
  geom_ribbon(data = dummy, aes(x = pchange0019, ymin = ftp0019-stp0019, ymax = ftp0019+stp0019, fill = dist_type3), alpha = .4)+
  geom_line(data = dummy, aes(pchange0019, ftp0019, col = dist_type3), lwd = 1.3)+
  ...
  labs(x = expression(paste('MAP change 2000-2019 (mm ' ~year^-1, ')')),
       y = expression(paste('TC change (% ' ~"%"^-1 ~year^-1, ')')),
       tag = "I")+
  theme_classic()+
  theme(legend.key.width = unit(2, units = "cm"),
...
plottpt = ggarrange(distplot, tcplot, elevplot.rel, matplot, mapplot, ttplot, tpplot, tt0019plot, tp0019plot,
                    nrow = 3, 
                    ncol = 3,
                    common.legend = T,
                    legend = "bottom")
...
#### 5.2.2 Model ABSOLUTE TREE COVER and.... ####
#...distance by disturbance and vegetation type
M02 = gamm(trend.yp ~ 
             s(distance, by = dist_GLC),
           random = list(lon =~1),
...
#' Get residuals and fitted values:
canada$E02  <- resid(M02$gam)
canada$mu02 <- fitted(M02$gam)
canada$E11  <- resid(M11$gam)
canada$mu11 <- fitted(M11$gam)
canada$E12  <- resid(M12$gam)
canada$mu12 <- fitted(M12$gam)
canada$E13  <- resid(M13$gam)
canada$mu13 <- fitted(M13$gam)
canada$E14  <- resid(M14$gam)
canada$mu14 <- fitted(M14$gam)
canada$E15  <- resid(M15$gam)
canada$mu15 <- fitted(M15$gam)
canada$E16  <- resid(M16$gam)
canada$mu16 <- fitted(M16$gam)
canada$E17  <- resid(M17$gam)
canada$mu17 <- fitted(M17$gam)
canada$E18  <- resid(M18$gam)
canada$mu18 <- fitted(M18$gam)
canada$E19  <- resid(M19$gam)
canada$mu19 <- fitted(M19$gam)


#The code below can be used for validation of each model by changing the fitted and residual values, mux and Ex, as well as the predictor used in the model. 
#' Pearson residuals versus fitted values.

p1 <- ggplot(canada, aes(x = mu11, y = E11))+
  geom_point(size = 1)+
  geom_hline(yintercept = 0, 
             linetype="dashed",
             color = "red")+
...
#' Pearson residuals versus disturbance type
p2 <- ggplot(canada, aes(x = dist_type3, y = E11))+
  geom_boxplot()+
  geom_hline(yintercept = 0, color = "red")+
  labs(x = "Disturbance", y = "Residuals")+
  theme(text = element_text(size=15),
...
#' Fitted values versus observed data.
p3 <- ggplot(canada, aes(x = mu11, y = trend.yp))+
  geom_point(size = 0.5, col= grey(0.5))+
  geom_abline(intercept = 0, slope = 1, color = "red")+
  labs(x = "Fitted values", y = "Observed Tree cover change")+
  theme(text = element_text(size = 10))
...
#' Residuals versus predictor in the model
p4 <- ggplot(canada, aes(x = distance, y = E11))+
  geom_point()+
  geom_smooth()+
  geom_hline(yintercept = 0, color = "red")+
  labs(y = "Residuals")+
...
#' Residuals versus longitude
p5 <- ggplot(canada, aes(x = flon, y = E11))+
  geom_boxplot()+
  geom_hline(yintercept = 0, color = "red")+
  labs(x = "Longitude", y = "Residuals")+
  theme(text = element_text(size=15),
...
#### 5.2.2.2 Model prediction and visualisation####
#From dummy data predict and visualise...
#distance by vegetation type and disturbance
...
glcabsplot = 
  ...
  geom_point(data = canada, aes(distance, trend.yp, col = dist_type3), pch = 1, alpha = .4)+
  geom_line(aes(col = dist_type3), lwd = 1.3)+
  # geom_line(aes(distance, fdist + sdist), lwd = .5)+
  # geom_line(aes(distance, fdist - sdist), lwd = .5)+
  # 
...
allglcrawplots = ggarrange(glcabsplot, glcrelplot, nrow = 2, common.legend = F)
allglcrawplots
...
distplot = 
  ggplot(dummy, aes(distance, fdist))+
  geom_point(data = canada, aes(distance, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(distance, fdist + sdist), lwd = .5)+
  geom_line(aes(distance, fdist - sdist), lwd = .5)+
...
matplot = 
  ggplot(dummy, aes(temp, ftemp))+
  geom_point(data = canada, aes(temp, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(temp, ftemp + stemp), lwd = .5)+
  geom_line(aes(temp, ftemp - stemp), lwd = .5)+
...
mapplot = 
  ggplot(dummy, aes(meanpre, fpre))+
  geom_point(data = canada, aes(meanpre, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(meanpre, fpre + spre), lwd = .5)+
  geom_line(aes(meanpre, fpre - spre), lwd = .5)+
...
tcplot = 
  ggplot(dummy, aes(meantc, ftc))+
  geom_point(data = canada, aes(meantc, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(meantc, ftc + stc), lwd = .5)+
  geom_line(aes(meantc, ftc - stc), lwd = .5)+
...
elevplot = 
  ggplot(dummy, aes(elev, felev))+
  geom_point(data = canada, aes(elev, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(elev, felev + selev), lwd = .5)+
  geom_line(aes(elev, felev - selev), lwd = .5)+
...
ttplot = 
  ggplot(dummy, aes(tchange8019, ftt))+
  geom_point(data = canada, aes(tchange8019, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(tchange8019, ftt + stt), lwd = .5)+
  geom_line(aes(tchange8019, ftt - stt), lwd = .5)+
...
tpplot = 
  ggplot(dummy, aes(pchange8019, ftp))+
  geom_point(data = canada, aes(pchange8019, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(pchange8019, ftp + stp), lwd = .5)+
  geom_line(aes(pchange8019, ftp - stp), lwd = .5)+
...
tt0019plot = 
  ggplot(dummy, aes(tchange0019, ftt0019))+
  geom_point(data = canada, aes(tchange0019, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(tchange0019, ftt0019 + stt0019), lwd = .5)+
  geom_line(aes(tchange0019, ftt0019 - stt0019), lwd = .5)+
...
tp0019plot = 
  ggplot(dummy, aes(pchange0019, ftp0019))+
  geom_point(data = canada, aes(pchange0019, trend.yp), pch = 1, col = "grey", alpha = .6)+
  geom_line(lwd = 1.3)+
  geom_line(aes(pchange0019, ftp0019 + stp0019), lwd = .5)+
  geom_line(aes(pchange0019, ftp0019 - stp0019), lwd = .5)+
...
finalplot = grid.arrange(distplot, tcplot, elevplot, matplot, mapplot, ttplot, tpplot, tt0019plot, tp0019plot,
                         layout_matrix = rbind(c(1,1),
                                               c(2,3),
                                               c(4,5),
                                               c(6,7),
...
glcabsplot = 
  ggplot(dummyglc, aes(distance, fdist))+
  geom_ribbon(data = dummyglc, aes(x = distance, ymin = fdist-sdist, ymax = fdist+sdist, fill = dist_type3), alpha = .4)+
  geom_line(aes(col = dist_type3), lwd = 1.3)+
...
dummy$cat = as.factor("All disturbance types")
...
distplot = 
  ...
  labs(x = "Stand. boundary distance",
       y = expression(paste('Fitted tree cover change (% '~year^-1, ')')),
       tag = "B")+
  theme_classic()+
  theme(legend.key.width = unit(1, units = "cm"),
        text = element_text(size = 25),
        panel.spacing = unit(2, "lines"),
        strip.background = element_rect(fill = "black"),
        strip.text = element_text(colour = "white", face = "bold"),
...
matplot = 
  ...
  labs(x = "Mean annual temperature 1980-2019 (°C)",
       y = expression(paste('TC change (% ' ~year^-1, ')')),
       tag = "C")+
  theme_classic()+
  theme(legend.key.width = unit(2, units = "cm"),
...
mapplot = 
  ...
  labs(x = "Mean annual precipitation 1980-2019 (mm)",
       y = expression(paste('TC change (% ' ~year^-1, ')')),
       tag = "D")+
  theme_classic()+
  theme(legend.key.width = unit(2, units = "cm"),
...
finalplot = ggarrange(tcplot, elevplot, matplot, mapplot, ttplot, tpplot, tt0019plot, tp0019plot, nrow = 4, ncol = 2, common.legend = T, legend = "bottom")
