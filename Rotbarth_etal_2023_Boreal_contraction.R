#############################Introduction######################################
# R script containing data preparation, exploration, visualisation and analysis for the paper titled 'Are North American boreal forests contracting?' published in Nature Communications 2023
# 
...
# 
# R script version dated 4 April 2023
# 
...
# 
# 
# Paper abstract: Climate change is expected to shift the boreal biome northward through expansion at the northern and contraction at the southern boundary respectively. However, biome-scale evidence of such a shift is rare. We used remotely-sensed tree cover data to quantify temporal changes across the North American boreal biome over the past two decades (2000-2019). Rather than shifting biome borders, we reveal a strong north-south asymmetry in tree cover change, coupled with a range shrinkage of tree cover distributions. In the northern biome, we found no evidence for tree cover expansion, while tree cover increased markedly in the core of the biome range. By contrast, tree cover declined along the southern biome boundary, where losses were related largely to wildfires and timber logging. The lack in tree cover expansion at the northern biome boundary and the losses observed in the south are structural indicators for the onset of a biome contraction. In the short run, our results suggest a trend for release of biomass carbon in the southern biome and an uptake of biomass carbon coupled with potential albedo-induced warming in the boreal interior. However, long-term carbon storage may decline under a contraction scenario. Continuous monitoring is therefore required to assess a climate change related biome contraction over longer time periods. 
# 
# 
# Data set to be used throughout is titled 'NA_boreal_data_v3.csv' 
...
#### Read in dataset ####
all.boreal = read_csv("NA_boreal_data_v3.csv")
...
all.boreal$boundary = as.factor(all.boreal$boundary) 
...
all.boreal$boundary = as.factor(all.boreal$boundary) 
all.boreal$boundary = factor(all.boreal$boundary, levels = c("N-North", "S-North", "N-Interior", "S-Interior", "N-South", "S-South"))

#### Data exploration: Correlation between predictors ####
variables = select(all.boreal, c(distance,elev,meantc,temp,meanpre,tchange8019,tschange8019,pchange8019,tchange0019,tschange0019,pchange0019))
...
canada = subset(all.boreal, all.boreal$ecozone != "Alaska")
...
canada$dist_type = factor(canada$dist_type, levels = c("Undisturbed", "Fire", "Harvest"))


# all model results are saved as RDS files and are provided, as the modeling process is quite time consuming. The expressions of each model are listed below. 

all.models = read_rds("boreal_models_AK.rds")
...
dummyglc$dist_type = factor(dummyglc$dist_type, levels = levels(canada$dist_type))
